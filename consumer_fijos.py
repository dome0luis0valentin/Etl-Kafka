...
if not os.path.exists("./datos_fijos_iniciales/"):
    #crea el directorio
    os.makedirs("./datos_fijos_iniciales/")
...
with open("resultado.txt", "a") as file:
    file.write(f"Resultado Plazo Fijo y Doláres\n")
    file.write(f"Fecha: {datetime.now()}\n")
    file.write(f"El total de la variación porcentual del dólar blue fue de: {total_percentage_usd:.2f}%  \n")
    file.write(f"Tu inversión en plazo fijo a generado: {total_plazo_fijo:.2f}%")
    file.write("\n\n\n\n\n")